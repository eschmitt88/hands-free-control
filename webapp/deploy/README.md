...
collector. Runs on <build-server> as a `--user` systemd unit, HTTPS on :8104.
...
is signed by the **<other-project> local CA** — clients that already trust that rootCA
(e.g. <reviewer-desktop>) need no new import. Regenerate with:
```sh
# openssl req/x509 signing against ~/projects/<other-project>/certs/rootCA.pem
# SANs: <build-server>, <hostname>.<tailnet>.ts.net, <tailnet-IP>, <LAN-IP>, localhost, 127.0.0.1
```
`webapp/certs/` and `webapp/static/vendor/` are gitignored (keys + fetched model).

## Access
- Tailscale/LAN: https://<build-server>:8104/  (or https://<tailnet-IP>:8104/)
